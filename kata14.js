...
const calculateChange = (total, cash) => {
  ...
  const changeToGive = {};
  if (changeTotal > 2000) {
    const array = calculateDenomination(changeTotal, 2000);
    changeToGive["twentyDollar"] = array[0];
    changeTotal = array[1];
  }
  if (changeTotal > 1000) {
    const array = calculateDenomination(changeTotal, 1000);
    changeToGive["tenDollar"] = array[0];
    changeTotal = array[1];
  }
  if (changeTotal > 500) {
    const array = calculateDenomination(changeTotal, 500);
    changeToGive["fiveDollar"] = array[0];
    changeTotal = array[1];
  }
  if (changeTotal > 200) {
    const array = calculateDenomination(changeTotal, 200);
    changeToGive["twoDollar"] = array[0];
    changeTotal = array[1];
  }
  if (changeTotal > 100) {
    const array = calculateDenomination(changeTotal, 100);
    changeToGive["oneDollar"] = array[0];
    changeTotal = array[1];
  }
  if (changeTotal > 25) {
    const array = calculateDenomination(changeTotal, 25);
    changeToGive["quarter"] = array[0];
    changeTotal = array[1];
  }
  if (changeTotal > 10) {
    const array = calculateDenomination(changeTotal, 10);
    changeToGive["dime"] = array[0];
    changeTotal = array[1];
  }
  if (changeTotal > 5) {
    const array = calculateDenomination(changeTotal, 5);
    changeToGive["nickle"] = array[0];
    changeTotal = array[1];
  }
  if (changeTotal > 1) {
    const array = calculateDenomination(changeTotal, 1);
...
const calculateDenomination = (amount, denomination) => {
  const change = amount / denomination;
  const remainderCash = Math.round((change - Math.trunc(change)) * denomination);
  const numberWholeBills = Math.trunc(change);
